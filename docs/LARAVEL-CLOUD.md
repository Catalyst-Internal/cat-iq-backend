...
Laravel Cloud can run the scheduler cron. Ensure the platform scheduler invokes `php artisan schedule:run` every minute. This app schedules:

- `github:sync-org` every six hours
- `RefreshGitHubCacheJob` every five minutes (300s cache keys) and every minute for the `activity` key (60s TTL)
...
Mirror Cloud values locally for debugging: `GITHUB_APP_ID`, `GITHUB_APP_INSTALLATION_ID`, `GITHUB_APP_PRIVATE_KEY` or `GITHUB_APP_PRIVATE_KEY_BASE64`, `GITHUB_WEBHOOK_SECRET`, `DASHBOARD_AUTH_USER`, `DASHBOARD_AUTH_PASSWORD`, `FRONTEND_URL`, `SANCTUM_STATEFUL_DOMAINS`, `STATAMIC_LICENSE_KEY`, database credentials, `APP_KEY` (never regenerate on Cloud if already set).
